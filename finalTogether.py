...
outputFile = dataFolder + 'tr_exp01.csv' # output 
...
with open(inputFile2, 'r') as file2:
    for line in file2:
        ...
        for tid in splitLine[1].split(','):
            mapping[tid] = (splitLine[2], splitLine[3], splitLine[0]) # tpm, fpkm, gene_id


# print(mapping)

with open(inputFile1, 'r') as file1, open(outputFile, 'w') as output:
    output.write('%s,%s,%s,%s,%s,%s,%s\n' %('chromosome', 'start', 'end', 'transcript_id', 'strand', 'asinh_TPM', 'gene_id'))
    for line in file1:
        splitLine = line.split('\t')
        id = splitLine[7]
        if id in mapping:
            output.write('%s,%s,%s,%s,%s,%s,%s\n' %(splitLine[4],splitLine[5],splitLine[6],splitLine[7],splitLine[9], mapping[id][0], mapping[id][2]))
